 ...
 */
require(['jquery', 'util', 'Operator', 'Fact', 'control'],
        function($, util, Operators, Fact, control) {
            util.addPrototypes();
            // Utility functions
            /**
            ...
            function testToStringFamily(factString, expectedString, functionString, assert) {
                var fact = Fact.getFactFromString(factString);
            ...
            QUnit.test("Fact parsing - Single", function(assert) {
                var fact = Fact.getFactFromString("p");
            ...
            QUnit.test("Fact parsing - Basic", function(assert) {
                var fact = "p > q";
                var conditional = Fact.getFactFromString("(p>q)");
            ...
            QUnit.test("Fact parsing - Complex", function(assert) {
                var fact = "~p > (q # (r & ~s))";
                var complex = Fact.getFactFromString("(~(p)>(q#(r&~(s))))");
                // Check the operator
            ...
            QUnit.test("Fact.toParsableString", function(assert) {
                // Reflective test
                var fact = Fact.getFactFromString("((~(s)|b)#~((a&(c>b))))");
                var reflect = Fact.getFactFromString(fact.toParsableString());
            ...
            QUnit.test("Fact.getCopy", function(assert) {
                ...
                ref1 = Fact.getFactFromString("(a|b)");
                ref2 = ref1;
                ...
                ref1 = Fact.getFactFromString("(p&q)");
                ref2 = ref1;
            ...
            QUnit.test("Fact.getInverse", function(assert) {
                ...
                // The p scenario
                fact = Fact.getFactFromString("(a|~(c))");
                expecting = Fact.getFactFromString("~((a|~(c)))");
                manipulated = fact.getInverse();
                assert.deepEqual(manipulated, expecting, "The inverse of p is ~p");

                // The ~p scenario
                fact = Fact.getFactFromString("~((a&(c|d)))");
                expecting = Fact.getFactFromString("(a&(c|d))");
                manipulated = fact.getInverse();
                assert.deepEqual(manipulated, expecting, "The inverse of ~p is p");

                // The ~~p scenario
                fact = Fact.getFactFromString("~(~(p))");
                expecting = Fact.getFactFromString("~(p)");
                manipulated = fact.getInverse();
                assert.deepEqual(manipulated, expecting, "The inverse of ~~p is ~p");
            });
            ...
            QUnit.test("Fact.getNegation", function(assert) {
                ...
                // The p scenario
                fact = Fact.getFactFromString("(a|b)");
                expecting = Fact.getFactFromString("~((a|b))");
            ...
            /*
             * Test the Fact.createFactFromComponents factory function
             */
            QUnit.test("Fact.createFactFromComponents", function(assert) {
                // Init
                var fact, arg0, arg1, op;

                // Do the test
                arg0 = Fact.getFactFromString("(a|b)");
                arg1 = Fact.getFactFromString("(a|c)");
                op = Operators.AND;
                fact = Fact.createFactFromComponents(arg0, arg1, op);
             ...
             */
            QUnit.test("Fact.getFactFromHTMLString", function(assert) {
                /**
                ...
                function test(factString) {
                    // Create the fact
                    var expectedFact = Fact.getFactFromString(factString);

                    // Get the Html Fact
                    control.generateFactHTML(expectedFact, $('#util'), 'test', function() {
                    });
                    var actualFact = Fact.getFactFromHTMLString($('#util').text(), false);
