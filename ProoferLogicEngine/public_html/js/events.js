            ...
            var events = {
                init: function() {
                    $('#MenuReset').click(function() {
                        ...
                        cloud.clearData();
                        Levels.clearState();
                        cloud.saveDataSync("currentLevel", {
                            index: 0
                        }, function() {
                    ...
                    $('#Controls_Executor_Save').click(function() {
                        ...
                        cloud.getData("currentLevel", function(data) {
                            data.progress = cloud.getSaveLevelObj(Levels.current);
                            cloud.saveDataSync("currentLevel", data, function() {
                    ...
                    $('#Controls_Executor_Evaluate').click(function() {
                        ...
                        // Get the rule
                        var rule = events.getRuleFromContainer($('#Controls_Executor_Rule'), level);
                        if (rule === null) {
                        ...
                        // Get the result
                        var result = (Rules.isUnaryRule(rule)) ?
                        ...
                        if (result !== null) {
                            ...
                            $('#Controls_Executor_Result').click(function() {
                                ...
                                if (!factExists) {
                                    ...
                                    // Update the table
                                    $('#Controls_Facts_Table').append(control.generateFactRow(index, result.toPrettyString()));

                                    // Make it draggable
                                    events.bindFactEvents();
                                }

                                // Clear the results and input
                                control.clearExecutorResult();
                                control.clearExecutorInputs();

                                // Remove the event, it's no longer valid
                                $('#Controls_Executor_Result').unbind("click");

                                // Check if it's the conclusion
                                events.checkForLevelCleared(result);
                            });
                        } else {
                    ...
                    $('#Controls_Modifier_AddIt').click(function() {
                        ...
                        var fact = Fact.getFactFromHTMLString($(".glowing").text(), true);
                        ...
                        if (!factExists) {
                            ...
                            // Update the table
                            $('#Controls_Facts_Table').append(control.generateFactRow(index, fact.toPrettyString()));

                            // Make it draggable
                            bindFactEvents();

                            // Check if it's the end
                            events.checkForLevelCleared(fact);
                        }
                 ...
                 */
                getRuleFromContainer: function($container, level) {
                    // Init
                    var rule = null;
                 ...
                 */
                updateSelectLevelEvents: function() {
                ...
                updateReplacementResults: function() {
                    ...
                    fact = Fact.getFactFromHTMLString(factString, false);

                    // Now compute the results
                    results = [];
                    if (Rules.isAmbiguousRule(rule)) {
                    ...
                    if (results.length === 0) {
                        ...
                        for (var i = 0; i < results.length; i++) {
                            ...
                            if (newFact.find(".replacement-control-selected").length === 0) {
                                ...
                                control.generateFactHTML(results[i], newFact.find(".replacement-control-selected"), 'replacement', function() {
                                });
                                newFact.find(".replacement-control-selected").children().unwrap();
                                newFact = Fact.getFactFromHTMLString(newFact.text(), false);
                            }
                 ...
                 * @returns {undefined}
                 */
                bindFactDetailResultEvents: function() {
                ...
                bindFactDetailEvents: function() {
                    ...
                    $('.replacement-control').click(function(event) {
                        ...
                        // Update the results
                        events.updateReplacementResults();
                    });
                },
                ...
                updateCreationElements: function() {
                ...
                displayNewFactSelector: function(event) {
                    // Get position
                    var top = event.pageY;
                        ...
                        drop: function(event) {
                            ...
                            if (droppedItem.getAttribute('ruleId') !== null) {
                                ...
                                if (target.attr("id") === "Controls_Executor_Rule") {
                                    if (Rules.isUnaryRule(Rules[droppedItem.getAttribute('ruleId')])) {
