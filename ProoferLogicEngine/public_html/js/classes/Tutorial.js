...
var Tutorials = {
    ...
    Add_INTRO: new Tutorial([
        new Page(
                '<h3>The new rule</h3><p>The rule of addition states that if you ' +
                'know p, then you know p or (inclusive) anything else is true. ' +
                'After all, with an inclusive or you only need one thing to be ' +
                'true, and you\'ve already got it. It\'s another of those common ' +
                'sense rules that is used mostly as a formality.</p><h3>The form' +
                '</h3><ul style="list-style-type: none;"><li>p</li><li>therefore ' +
                'p&or;q</li></ul><h3>Summary</h3><p>You might be wondering, "how ' +
                'do I get my q?" You\'ll see a placeholder appear where you usually ' +
                'put your second statement. Click that, and you\'ll be introduced to ' +
                'the Add Table, which will help you work with the Statement Creator. ' +
                'Give it a try!</p>',
                'Addition')
    ],
            'New Rule'),
    Inference_Set_1: new Tutorial([
        new Page(
                '<h3>Objective</h3><p>You\'ve completed the first set of tutorials, ' +
                'congratulations. You\'ll be on your own for a bit now, and each ' +
                'level will actually present some kind of a challenge.</p><h3>And ' +
                'after that?</h3><p>Once you have mastered this first set of problems, ' +
                'you will be given a new tool - one that will add some much needed ' +
                'flexibility but will consequently complicate everything you thought ' +
                'you knew about logic. Good luck :)</p>',
                'You\'re on your own')
    ],
    'Now What?')
};